---
title: "memote_clustering_metric_data_R"
...
library(cluster) # pam
library('ape')
df=read.csv('data-clustering-metric-data-R/memote_clustering_table.tsv',sep='\t')
...
plot.rf.var.importance.by.class.andMean.dotplot <- function(model,predVar,classVar,colorVector=NULL,nBestFeatures=NULL){
  imp.df=melt(importance(model)[,1:(length(model$classes)+1)])
  colnames(imp.df)=c(predVar,classVar,'value')
  # a.-Order rows
  pred.order=names(sort(importance(model)[,'MeanDecreaseAccuracy'])) # My order according to global MeandDecreaseAccuracy
...
#model <- train(form=collection~.,data=df.rf, trControl=train_control, method="rf",ntree=1000,importance=TRUE,localImp=TRUE,na.action=na.omit)
#save(model,file="data-clustering-metric-data-R/rf_model_classCollection.Rdata")
load('data-clustering-metric-data-R/rf_model_classCollection.Rdata')
```
...
plot.rf.var.importance.by.class.andMean.dotplot(model,'test','collection',colorVec=c(colsCollection,'#60d660'),nBestFeatures=15)
```

## Clustering
```{r pamk, eval=FALSE}
fitPamBest <- pamk(df.num,krange=2:25)
save(fitPamBest,file='data-clustering-metric-data-R/fitPamBest_k2-25.Rdata')
write.table(as.matrix(fitPamBest$pamobject$clustering),paste("data-clustering-metric-data-R/pam_clusters_k",fitPamBest$nc,".txt",sep=""),quote=FALSE,sep='\t',col.names=NA,row.names=TRUE)
...
write.table(as.matrix(groups),paste("data-clustering-metric-data-R/hclust_clusters_k",kbest,".txt",sep=""),quote=FALSE,sep='\t',col.names=NA,row.names=TRUE)
```
...
#set.seed(123)
#model.cl <- train(form=cluster~.,data=df.rf, trControl=train_control, method="rf",ntree=1000,importance=TRUE,localImp=TRUE,na.action=na.omit,do.trace=10)
#save(model.cl,file="data-clustering-metric-data-R/rf_model_classCluster.Rdata")
load("data-clustering-metric-data-R/rf_model_classCluster.Rdata")
```
...
```{r fig.width=10, fig.height=18}
library(reshape2)
colVector=c("#A6A9AA","#000000","#000000","#000000","#8B4513","#A3D2E2","#8B4513","#7E8082","#EDA85F","#EDA85F",'#60d660')
