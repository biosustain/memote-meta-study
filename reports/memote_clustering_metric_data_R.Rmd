...
library(cluster) # pam
library('ape')
df=read.csv('memote_clustering_table.tsv',sep='\t')
...
#save(model,file="rf_model_classCollection.Rdata")
load('rf_model_classCollection.Rdata')
```
...
save(fitPamBest,file='fitPamBest_k2-25.Rdata')
write.table(as.matrix(fitPamBest$pamobject$clustering),paste("pam_clusters_k",fitPamBest$nc,".txt",sep=""),quote=FALSE,sep='\t',col.names=NA,row.names=TRUE)
...
write.table(as.matrix(groups),paste("hclust_clusters_k",kbest,".txt",sep=""),quote=FALSE,sep='\t',col.names=NA,row.names=TRUE)
```
...
#save(model.cl,file="rf_model_classCluster.Rdata")
load("rf_model_classCluster.Rdata")
```
