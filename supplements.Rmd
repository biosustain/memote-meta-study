...
* SBO terms are only used consistently by models from KBase and BiGG (see Figures \@ref(fig:annotation-sbo-score)).
...
* Models from the collections of Ebrahim \textit{et al.} and OptFlux Models are highly variable for many specific tests. Models from automatic reconstruction pipelines (AGORA, CarveMe, Path2Models and KBase) or the controlled BiGG collection are much more similar within each collection yet still different from each other. This could be due to each collection focusing on a distinct set of taxonomies but could also be related to the algorithms and databases behind each collection (Section \@ref(sec:network-topology)).
* On biomass:
    ...
    * A portion of models in the BiGG collections have inconsistent biomass equations followed by OptFlux Models and models in the collection by Ebrahim \textit{et al.} (Figure \@ref(fig:biomass-consistency)).
...
metric_plots <- total_df %>%
  dplyr::filter(is.finite(metric), is.finite(numeric)) %>%
  dplyr::group_by(test) %>%
  dplyr::do(
...
```{r consistency-score, fig.cap='Consistency. Depicted are the sums of all test scores in this section, assuming a uniform weight across the tests.'}
...
```{r annotation-met-score, fig.cap='Annotation - Metabolites. Depicted are the sums of all test scores in this section, assuming a uniform weight across the tests.'}
...
```{r annotation-rxn-score, fig.cap='Annotation - Reactions. Depicted are the sums of all test scores in this section, assuming a uniform weight across the tests.'}
...
```{r annotation-gene-score, fig.cap='Annotation - Genes. Depicted are the sums of all test scores in this section, assuming a uniform weight across the tests.'}
...
```{r annotation-sbo-score, fig.cap='Annotation - SBO Terms. Depicted are the sums of all test scores in this section, assuming a uniform weight across the tests.'}
